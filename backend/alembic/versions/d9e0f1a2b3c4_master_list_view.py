...
SELECT
    ...
    (SELECT COALESCE(SUM(cl.qty), 0)
       FROM container_lines cl WHERE cl.container_id = c.id)
                                          AS units,
    -- Pallet count = direct count of pallet rows for this container.
    -- (lot_assignments only tracks planned vs actual pallet counts as ints,
    -- it doesn't FK each pallet — the pallets table holds those.)
...
SELECT
    ...
    -- scans.pallet_id is set when the scan was palletized at receipt time.
